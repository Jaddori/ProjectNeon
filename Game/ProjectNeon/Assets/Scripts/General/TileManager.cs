...
public class TileManager : MonoBehaviour
{
	...
	public void InstantiatePrefabs()
	{
		...
		for( int curLayer = 0; curLayer < room.Layers.Count; curLayer++ )
		{
			...
			List<TileLayer> groupLayers = room.Layers.Where( x => !string.IsNullOrEmpty( x.Group ) && x.Group == layer.Group ).ToList();
			...
			for( int curRow = 0; curRow < layer.Rows.Count; curRow++ )
			{
				...
				for( int curTile = 0; curTile < row.Tiles.Count; curTile++ )
				{
					...
					if( _parser.Aliases.ContainsKey( tmp ) )
					{
						TileAlias alias = _parser.Aliases[tmp];

						var position = new Vector3( curTile * 10, 0, curRow * 10 );

						int sideIndex = alias.Parts[TileAlias.SIDE];
						int cornerIndex = alias.Parts[TileAlias.CORNER];
						int topIndex = alias.Parts[TileAlias.TOP];
